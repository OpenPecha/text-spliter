...
class LineByLineProcessor:
    def __init__(self, input_dir: Path, output_dir: Path, content_threshold: int = 50):
        ...
        self.content_threshold = content_threshold  # Minimum meaningful lines to trigger split
    ...
    def count_meaningful_lines_before_section(self, lines: List[str]) -> Tuple[int, int]:
        """
    ...
    def process_file(self, input_file: Path) -> List[Path]:
        ...
        meaningful_count, first_section_line = self.count_meaningful_lines_before_section(lines)
        should_split_before_first = meaningful_count >= self.content_threshold and first_section_line > 0
        ...
        for line in lines:
                ...
                if section_counter == 0:
                    if should_split_before_first:
                        ...
                        if last_page_marker_line is not None:
                            processed_marker_line = BRACKET_PAGE_RE.sub("Page: 1", last_page_marker_line)
            ...
            if has_page_marker:
                processed_line, page_counter = self.replace_page_markers(processed_line, page_counter)
    ...
    def write_section_file(self, input_file: Path, section: str, lines: List[str]) -> Path:
        """
...
def main():
    ...
    input_dir = script_dir / "data" / "kagyur_text"
    output_dir = script_dir / "data" / "kagyur_output"
