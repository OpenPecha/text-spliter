...
class DriveDocumentLinker:
    ...
    def __init__(self, credentials_path: str, tengyur_folder_id: str, token_path: str = "token.json"):
        """
    ...
    def get_all_document_links(self) -> Dict[str, str]:
        ...
        """
        self.logger.info(f"📂 Starting document retrieval from Tengyur folder\n   Folder ID: {self.tengyur_folder_id}")
        ...
        try:
            ...
            if not filename_to_link:
                self.logger.info("🔍 No Google Docs found. Checking for ANY files in the folder...")
                ...
                if all_files:
                    ...
                    for file in all_files[:5]:
                        self.logger.info(f"   • {file['name']} (Type: {file['mimeType']})")
            
            # Log sample of what we found
            if filename_to_link:
                sample_files = list(filename_to_link.keys())[:5]
                self.logger.info(f"📋 Sample Google Docs found:\n   {chr(10).join([f'   • {file}' for file in sample_files])}")
            
            return filename_to_link
            
        except HttpError as e:
            self.logger.error(f"❌ DRIVE API ERROR: Failed to fetch documents\n   Error details: {e}\n   Folder ID: {self.tengyur_folder_id}")
            return {}
    
    def display_results(self, filename_to_link: Dict[str, str]):
        """
    ...
    def save_to_json(self, filename_to_link: Dict[str, str], output_file: str = "document_links.json"):
        ...
        try:
            ...
            self.logger.info(f"💾 Successfully saved {len(filename_to_link)} document links\n   Output file: {output_file}")
            print(f"\n💾 Results saved to: {output_file}")
        except Exception as e:
            self.logger.error(f"❌ FILE SAVE ERROR: Failed to save document links to JSON\n   Target file: {output_file}\n   Error details: {e}")
