...
class GoogleDocsUploader:
    ...
    def __init__(self, credentials_path: str, progress_file: str = "upload_progress.json", token_path: str = "token.json"):
        """
    ...
    def get_tengyur_folder(self) -> str:
        ...
        try:
            ...
            self.logger.info(f"📂 Using existing Tengyur folder\n   Name: {folder_info['name']}\n   ID: {self.tengyur_folder_id}\n   Location: {where}")
            self.progress_data['tengyur_folder_id'] = self.tengyur_folder_id
            self.save_progress()
            return self.tengyur_folder_id
        except HttpError as e:
            self.logger.error(f"Failed to access Tengyur folder {self.tengyur_folder_id}: {e}")
            raise
    ...
    def read_text_file(self, text_id: str) -> Optional[str]:
        ...
        text_file_path = Path(f"../../data/text/{text_id}")
    ...
    def create_google_doc(self, text_id: str, content: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.info(f"🎉 Successfully created Google Doc\n   Text ID: {text_id}\n   Doc ID: {doc_id}")
            return doc_id
    ...
    def get_text_ids_in_range(self, start_id: str = None, end_id: str = None) -> List[str]:
        """Get list of text IDs in specified range"""
        # Read all text IDs from text_list.txt
        text_list_path = Path('../../data/text_list.txt')
        ...
        if start_id or end_id:
            try:
                ...
                end_idx = all_text_ids.index(end_id) + 1 if end_id else len(all_text_ids)
                filtered_ids = all_text_ids[start_idx:end_idx]
                self.logger.info(f"Processing range: {start_id or 'start'} to {end_id or 'end'} ({len(filtered_ids)} files)")
                return filtered_ids
    ...
    def upload_batch(self, start_id: str = None, end_id: str = None, delay: float = 1.0):
        ...
        completed_count = len([tid for tid in text_ids if tid in self.progress_data['completed']])
        ...
        for i, text_id in enumerate(text_ids, 1):
            try:
                ...
                if success:
                    self.logger.info(f"🚀 Overall Progress: {i}/{total_files} files completed")
                else:
                    self.logger.error(f"⚠️  Overall Progress: {i}/{total_files} files processed (last failed)")
        ...
        # Final summary
        final_completed = len([tid for tid in text_ids if tid in self.progress_data['completed']])
        final_failed = len([tid for tid in text_ids if tid in self.progress_data['failed']])
...
def main():
    ...
    try:
        ...
        print("\n🔄 Initializing upload...")
        uploader = GoogleDocsUploader(config['credentials_path'], config['progress_file'])
