import unittest
from src.kagyur_text_splitter.kagyur_splitter import LineByLineProcessor
...
class TestSimpleInputOutput(unittest.TestCase):
    ...
    def test_process_line_examples(self):
        ...
        for input_text, expected_output, expected_section, expected_has_page in test_cases:
            with self.subTest(input_text=input_text):
                processed, section, has_page = self.processor.process_line(input_text)
                self.assertEqual(processed, expected_output, f"Output mismatch for: {input_text}")
                self.assertEqual(section, expected_section, f"Section mismatch for: {input_text}")
                self.assertEqual(has_page, expected_has_page, f"Page detection mismatch for: {input_text}")
    ...
    def test_replace_page_markers_examples(self):
        ...
        for input_text, start_counter, expected_output, expected_counter in test_cases:
            with self.subTest(input_text=input_text):
                processed, new_counter = self.processor.replace_page_markers(input_text, start_counter)
                self.assertEqual(processed, expected_output, f"Output mismatch for: {input_text}")
                self.assertEqual(new_counter, expected_counter, f"Counter mismatch for: {input_text}")
    ...
    def test_meaningful_line_examples(self):
        ...
        for input_text, expected_meaningful in test_cases:
            with self.subTest(input_text=input_text):
                ...
                self.assertEqual(result, expected_meaningful, f"Meaningful detection failed for: '{input_text}'")
